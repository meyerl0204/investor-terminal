...
with tab2:
    ...
    def fetch_openinsider():
        ...
        url = (
            ...
            "s=&o=&pl=&ph=&ll=&lh=&fd=60&fdr=&td=0&tdr=&fdlyl=&fdlyh=&daysago=&xp=1"
    ...
    # ── Fetch fundamentals for all tickers in the insider df ─────────────────
    @st.cache_data(ttl=3600)
    def fetch_fundamentals(tickers):
        """Pull key fundamental metrics from yfinance for a list of tickers."""
        results = {}
        for sym in tickers:
            try:
                info = yf.Ticker(sym).info
                # Revenue growth (yfinance gives trailing 12mo vs prior year)
                rev_growth   = info.get("revenueGrowth")        # e.g. 0.18 = 18%
                # Profit margins
                net_margin   = info.get("profitMargins")         # e.g. 0.12 = 12%
                ...
                results[sym] = {
                    ...
                    "Net Margin %":    round(net_margin * 100, 1)    if net_margin    is not None else None,
                    "Gross Margin %":  round(gross_margin * 100, 1)  if gross_margin  is not None else None,
                    "Fwd P/E":         round(fwd_pe, 1)              if fwd_pe        is not None else None,
                    "P/S":             round(ps_ratio, 1)            if ps_ratio      is not None else None,
                    "P/B":             round(pb_ratio, 1)            if pb_ratio      is not None else None,
                    "EPS Growth %":    round(earnings_growth * 100, 1) if earnings_growth is not None else None,
                    "ROE %":           round(roe * 100, 1)           if roe           is not None else None,
                    "Debt/Equity":     round(debt_equity, 1)         if debt_equity   is not None else None,
                }
            except Exception:
                results[sym] = {}
        return results
    ...
    if insider_df.empty:
        ...
        unique_syms = tuple(insider_df["Ticker"].unique())
        with st.spinner("Loading fundamentals data..."):
            fund_map = fetch_fundamentals(unique_syms)

        # Merge fundamentals into df
        for col in ["Rev Growth %", "Net Margin %", "Gross Margin %", "Fwd P/E", "P/S", "P/B", "EPS Growth %", "ROE %", "Debt/Equity"]:
            insider_df[col] = insider_df["Ticker"].map(lambda t: fund_map.get(t, {}).get(col))
        ...
        with sfc1:
            sort_by = st.selectbox(
                "Sort by",
                ["Date (newest first)", "Ticker A → Z", "Dollar Amount (largest first)",
                 "Market Cap (largest first)", "Revenue Growth (highest)", "Net Margin (highest)", "Fwd P/E (lowest)"],
        ...
        # ── Fundamentals filter expander ───────────────────────────────────
        with st.expander("📊 Fundamentals Filter", expanded=False):
            st.caption("Filter to companies meeting minimum quality/valuation thresholds. Leave a slider at its minimum to skip that filter.")
            fc1, fc2, fc3 = st.columns(3)
            with fc1:
                min_rev_growth = st.slider("Min Revenue Growth %", min_value=-50, max_value=100, value=-50, step=5, key="f_rev")
                min_net_margin = st.slider("Min Net Margin %",      min_value=-50, max_value=50,  value=-50, step=5, key="f_margin")
                min_gross_margin = st.slider("Min Gross Margin %",  min_value=-10, max_value=90,  value=-10, step=5, key="f_gross")
            with fc2:
                max_fwd_pe   = st.slider("Max Forward P/E",  min_value=5,  max_value=200, value=200, step=5,  key="f_pe")
                max_ps       = st.slider("Max P/S Ratio",    min_value=1,  max_value=50,  value=50,  step=1,  key="f_ps")
                max_pb       = st.slider("Max P/B Ratio",    min_value=1,  max_value=30,  value=30,  step=1,  key="f_pb")
            with fc3:
                min_eps_growth = st.slider("Min EPS Growth %", min_value=-100, max_value=200, value=-100, step=10, key="f_eps")
                min_roe        = st.slider("Min ROE %",         min_value=-50,  max_value=100, value=-50,  step=5,  key="f_roe")
                max_de         = st.slider("Max Debt/Equity",   min_value=0,    max_value=500, value=500,  step=25, key="f_de")
        ...
        # Fundamentals filters — only apply if not at floor/ceiling (skip = no filter)
        def fund_filter(df, col, min_val_f=None, max_val_f=None):
            mask = df[col].notna()
            if min_val_f is not None:
                mask = mask & (df[col] >= min_val_f)
            if max_val_f is not None:
                mask = mask & (df[col] <= max_val_f)
            return df[mask | df[col].isna()]  # keep rows where data missing (don't penalise no data)

        if min_rev_growth   > -50:  display_df = fund_filter(display_df, "Rev Growth %",   min_val_f=min_rev_growth)
        if min_net_margin   > -50:  display_df = fund_filter(display_df, "Net Margin %",   min_val_f=min_net_margin)
        if min_gross_margin > -10:  display_df = fund_filter(display_df, "Gross Margin %", min_val_f=min_gross_margin)
        if max_fwd_pe       < 200:  display_df = fund_filter(display_df, "Fwd P/E",        max_val_f=max_fwd_pe)
        if max_ps           < 50:   display_df = fund_filter(display_df, "P/S",            max_val_f=max_ps)
        if max_pb           < 30:   display_df = fund_filter(display_df, "P/B",            max_val_f=max_pb)
        if min_eps_growth   > -100: display_df = fund_filter(display_df, "EPS Growth %",   min_val_f=min_eps_growth)
        if min_roe          > -50:  display_df = fund_filter(display_df, "ROE %",          min_val_f=min_roe)
        if max_de           < 500:  display_df = fund_filter(display_df, "Debt/Equity",    max_val_f=max_de)

        # Sort
        sort_map = {
            "Ticker A → Z":              ("Ticker",       True),
            "Dollar Amount (largest first)": ("Value ($)", False),
            "Market Cap (largest first)":("Market Cap",   False),
            "Revenue Growth (highest)":  ("Rev Growth %", False),
            "Net Margin (highest)":      ("Net Margin %", False),
            "Fwd P/E (lowest)":          ("Fwd P/E",      True),
        }
        if sort_by in sort_map:
            scol, sasc = sort_map[sort_by]
            display_df = display_df.sort_values(scol, ascending=sasc, na_position="last")
        else:
        ...
        if display_df.empty:
            ...
            # ── Card grid ──────────────────────────────────────────────────
            def _metric_pill(label, value, good_if_high=True, warn_thresh=None, good_thresh=None):
                """Render a small colored pill for a fundamental metric."""
                if value is None or (isinstance(value, float) and pd.isna(value)):
                    return f'<span style="background:#f0f0f0;color:#aaa;border-radius:4px;padding:2px 7px;font-size:10px;font-weight:600;">{label}: N/A</span>'
                if good_thresh is not None and warn_thresh is not None:
                    if good_if_high:
                        color = "#2ecc71" if value >= good_thresh else ("#f39c12" if value >= warn_thresh else "#e74c3c")
                    else:
                        color = "#2ecc71" if value <= good_thresh else ("#f39c12" if value <= warn_thresh else "#e74c3c")
                else:
                    color = "#555"
                return f'<span style="background:{color}22;color:{color};border:1px solid {color}55;border-radius:4px;padding:2px 7px;font-size:10px;font-weight:700;">{label}: {value}</span>'
            ...
            for chunk in chunks:
                ...
                for idx, (_, r) in enumerate(chunk.iterrows()):
                    ...
                    # Fundamental pills
                    rev_g   = r.get("Rev Growth %")
                    margin  = r.get("Net Margin %")
                    fwd_pe  = r.get("Fwd P/E")
                    ps      = r.get("P/S")
                    roe     = r.get("ROE %")

                    rev_g_str  = f"{rev_g:+.1f}%" if rev_g is not None and not (isinstance(rev_g, float) and pd.isna(rev_g)) else None
                    margin_str = f"{margin:.1f}%"  if margin is not None and not (isinstance(margin, float) and pd.isna(margin)) else None
                    pe_str     = f"{fwd_pe:.1f}x"  if fwd_pe is not None and not (isinstance(fwd_pe, float) and pd.isna(fwd_pe)) else None
                    ps_str     = f"{ps:.1f}x"      if ps is not None and not (isinstance(ps, float) and pd.isna(ps)) else None
                    roe_str    = f"{roe:.1f}%"      if roe is not None and not (isinstance(roe, float) and pd.isna(roe)) else None

                    pill_rev  = _metric_pill("Rev Gr",  rev_g_str  or None, good_if_high=True,  warn_thresh="skip", good_thresh="skip")
                    pill_mar  = _metric_pill("Margin",  margin_str or None, good_if_high=True,  warn_thresh="skip", good_thresh="skip")
                    pill_pe   = _metric_pill("Fwd P/E", pe_str     or None, good_if_high=False, warn_thresh="skip", good_thresh="skip")
                    pill_ps   = _metric_pill("P/S",     ps_str     or None, good_if_high=False, warn_thresh="skip", good_thresh="skip")
                    pill_roe  = _metric_pill("ROE",     roe_str    or None, good_if_high=True,  warn_thresh="skip", good_thresh="skip")

                    # Color-coded revenue growth pill
                    if rev_g is not None and not (isinstance(rev_g, float) and pd.isna(rev_g)):
                        rg_color = "#2ecc71" if rev_g >= 15 else ("#f39c12" if rev_g >= 5 else "#e74c3c")
                        pill_rev = f'<span style="background:{rg_color}22;color:{rg_color};border:1px solid {rg_color}55;border-radius:4px;padding:2px 7px;font-size:10px;font-weight:700;">Rev Gr: {rev_g:+.1f}%</span>'
                    # Color-coded margin pill
                    if margin is not None and not (isinstance(margin, float) and pd.isna(margin)):
                        mg_color = "#2ecc71" if margin >= 15 else ("#f39c12" if margin >= 5 else "#e74c3c")
                        pill_mar = f'<span style="background:{mg_color}22;color:{mg_color};border:1px solid {mg_color}55;border-radius:4px;padding:2px 7px;font-size:10px;font-weight:700;">Margin: {margin:.1f}%</span>'
                    # Color-coded P/E pill
                    if fwd_pe is not None and not (isinstance(fwd_pe, float) and pd.isna(fwd_pe)):
                        pe_color = "#2ecc71" if fwd_pe <= 15 else ("#f39c12" if fwd_pe <= 25 else "#e74c3c")
                        pill_pe  = f'<span style="background:{pe_color}22;color:{pe_color};border:1px solid {pe_color}55;border-radius:4px;padding:2px 7px;font-size:10px;font-weight:700;">Fwd P/E: {fwd_pe:.1f}x</span>'
                    # ROE pill
                    if roe is not None and not (isinstance(roe, float) and pd.isna(roe)):
                        roe_color = "#2ecc71" if roe >= 15 else ("#f39c12" if roe >= 8 else "#e74c3c")
                        pill_roe  = f'<span style="background:{roe_color}22;color:{roe_color};border:1px solid {roe_color}55;border-radius:4px;padding:2px 7px;font-size:10px;font-weight:700;">ROE: {roe:.1f}%</span>'
                    ...
                    with cols[idx]:
                        st.markdown(
                            ...
                            border-top:3px solid #2ecc71;">
                              ...
                              <div style="font-size:12px;color:#888;margin-bottom:10px;">{shares_str} shares @ {price_str}</div>
                              <div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:10px;">
                                {pill_rev}{pill_mar}{pill_pe}{pill_ps}{pill_roe}
                              </div>
...
with tab1:
    ...
    if st.session_state["analysis_done"]:
        ...
        def _series_to_chart(series, title, color, is_shares=False, is_eps=False, flip_neg=False, quarterly=False):
            ...
            # For metrics that are naturally negative (CapEx), flip ALL values consistently
            if flip_neg:
                s = s.abs()

            # Labels
            if quarterly:
                lbl = [pd.Period(x, freq='Q').strftime('Q%q %Y') for x in s.index]
            else:
                lbl = [str(x.year) for x in s.index]

            # Determine scale: use the max absolute value across ALL data points for consistent scaling
            max_abs = s.abs().max()
            if is_eps or is_shares:
                vals = [round(float(v), 4) for v in s.values]
                suffix = ""
            elif max_abs >= 1e9:
                vals = [round(float(v) / 1e9, 3) for v in s.values]
                suffix = "B"
            elif max_abs >= 1e6:
                vals = [round(float(v) / 1e6, 3) for v in s.values]
                suffix = "M"
            elif max_abs >= 1e3:
                vals = [round(float(v) / 1e3, 3) for v in s.values]
                suffix = "K"
            else:
                vals = [round(float(v), 2) for v in s.values]
                suffix = ""

            # Compute growth badges
            badges = []
            ...
            return {"id": title.lower().replace(" ","_"), "title": title, "type": "bar",
                    ...
                    "isShares": is_shares, "isEps": is_eps, "badges": badges}
        ...
        annual_charts = [
            ...
            _series_to_chart(_get_fin(financials, "Basic EPS", "Diluted EPS"), "EPS", "#9b59b6", is_eps=True),
        ...
        def _get_deferred_revenue(bs):
            """
            Try every key yfinance uses for deferred/contract revenue.
            If both current and long-term exist, sum them for total RPO proxy.
            """
            if bs is None or bs.empty:
                return None
            # All known yfinance key variants
            current_keys  = ["Current Deferred Revenue", "DeferredRevenue",
                              "Deferred Revenue", "ContractWithCustomerLiabilityCurrent",
                              "Deferred Income Current"]
            lt_keys       = ["Long Term Deferred Revenue", "Non Current Deferred Revenue",
                              "DeferredRevenueNonCurrent", "ContractWithCustomerLiabilityNonCurrent",
                              "Deferred Income Non Current"]
            combined_keys = ["Total Deferred Revenue", "Remaining Performance Obligation",
                              "RemainingPerformanceObligation"]

            # Try combined key first
            for k in combined_keys:
                if k in bs.index:
                    return bs.loc[k].sort_index()

            # Try to sum current + long-term
            curr = None
            lt   = None
            for k in current_keys:
                if k in bs.index:
                    curr = bs.loc[k].sort_index()
                    break
            for k in lt_keys:
                if k in bs.index:
                    lt = bs.loc[k].sort_index()
                    break

            if curr is not None and lt is not None:
                combined = curr.add(lt, fill_value=0)
        ...
        rpo = _get_deferred_revenue(balance_sheet)
        if rpo is not None and rpo.abs().max() > 0:
            annual_charts.append(_series_to_chart(rpo, "Deferred Rev / RPO", "#16a085"))
        ...
        quarterly_charts = [
            ...
            _series_to_chart(_get_fin(quarterly_fin, "Basic EPS","Diluted EPS"), "EPS (Q)", "#9b59b6", is_eps=True, quarterly=True),
        ...
        q_rpo = _get_deferred_revenue(q_balance_sheet)
        if q_rpo is not None and q_rpo.abs().max() > 0:
            quarterly_charts.append(_series_to_chart(q_rpo, "Deferred Rev / RPO (Q)", "#16a085", quarterly=True))
            ...
            tooltip: {{
              callbacks: {{
                label: c => {{
                  ...
                  if (ds.isEps) return "$" + v.toFixed(2);
                  if (ds.isShares) return Math.abs(v).toFixed(2) + (ds.suffix ? ds.suffix + " shares" : " shares");
                  if (ds.suffix === "B") return (v < 0 ? "-$" : "$") + Math.abs(v).toFixed(2) + "B";
                  if (ds.suffix === "M") return (v < 0 ? "-$" : "$") + Math.abs(v).toFixed(1) + "M";
                  if (ds.suffix === "K") return (v < 0 ? "-$" : "$") + Math.abs(v).toFixed(0) + "K";
                  return v.toFixed(2);
                }}
              }}
            }}
            ...
            y: {{
              ticks: {{
                ...
                callback: v => {{
                  ...
                  if (ds.isEps) return "$" + v.toFixed(2);
                  if (ds.isShares) return v.toFixed(1) + (ds.suffix || "");
                  if (ds.suffix === "B") return (v < 0 ? "-$" : "$") + Math.abs(v).toFixed(1) + "B";
                  if (ds.suffix === "M") return (v < 0 ? "-$" : "$") + Math.abs(v).toFixed(0) + "M";
                  if (ds.suffix === "K") return (v < 0 ? "-$" : "$") + Math.abs(v).toFixed(0) + "K";
                  return v;
                }}
              }},
