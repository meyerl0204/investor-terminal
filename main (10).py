  ...
  function switchRange(range) {{
    ...
    with kpi_tab1:
        ...
        with c7:
            ...
            for shares_key in ['Ordinary Shares Number', 'Share Issued', 'Common Stock']:
                try:
                    if balance_sheet is not None and shares_key in balance_sheet.index:
                        shares_data = balance_sheet.loc[shares_key].sort_index()
                        break
                except: pass
        ...
        with c8:
            capex_data = None
            for ck in ['Capital Expenditure', 'Purchase Of Property Plant And Equipment', 'Capital Expenditures']:
                if cashflow is not None and ck in cashflow.index:
                    capex_data = cashflow.loc[ck].sort_index()
                    if capex_data.mean() < 0: capex_data = capex_data * -1
                    break
        ...
        # RPO / Deferred Revenue (only shows for SaaS companies)
        rpo_data = None
        for rk in ['Remaining Performance Obligation', 'Deferred Revenue', 'DeferredRevenue']:
            if balance_sheet is not None and rk in balance_sheet.index:
                rpo_data = balance_sheet.loc[rk].sort_index()
                break
        if rpo_data is not None and not rpo_data.empty and rpo_data.abs().max() > 0:
            c_r1, _ = st.columns(2)
            with c_r1:
                ch, bg = bar_chart(rpo_data, "Deferred Rev", "#16a085")
                if ch:
                    st.subheader("📋 Remaining Performance Obligations")
                    st.caption("Contracted future revenue — higher = more predictable growth")
                    st.altair_chart(ch, use_container_width=True)
                    render_growth_badges(bg)
    ...
    with kpi_tab2:
        ...
        with c7:
            ...
            for shares_key in ['Ordinary Shares Number', 'Share Issued', 'Common Stock']:
                try:
                    if q_balance_sheet is not None and shares_key in q_balance_sheet.index:
                        q_shares_data = q_balance_sheet.loc[shares_key].sort_index()
                        break
                except: pass
        ...
        with c8:
            q_capex = None
            for ck in ['Capital Expenditure', 'Purchase Of Property Plant And Equipment', 'Capital Expenditures']:
                if quarterly_cf is not None and ck in quarterly_cf.index:
                    q_capex = quarterly_cf.loc[ck].sort_index()
                    if q_capex.mean() < 0: q_capex = q_capex * -1
                    break
            if q_capex is not None and not q_capex.empty:
                ch, bg = bar_chart(q_capex, "CapEx", "#8e44ad", quarterly=True)
                if ch:
        ...
        q_rpo = None
        for rk in ['Remaining Performance Obligation', 'Deferred Revenue', 'DeferredRevenue']:
            if q_balance_sheet is not None and rk in q_balance_sheet.index:
                q_rpo = q_balance_sheet.loc[rk].sort_index()
                break
        if q_rpo is not None and not q_rpo.empty and q_rpo.abs().max() > 0:
            c_r1, _ = st.columns(2)
            with c_r1:
                ch, bg = bar_chart(q_rpo, "Deferred Rev", "#16a085", quarterly=True)
                if ch:
                    st.subheader("📋 RPO / Deferred Revenue (Quarterly)")
                    st.caption("Contracted future revenue — rising trend signals strong pipeline")
                    st.altair_chart(ch, use_container_width=True)
                    render_growth_badges(bg)
