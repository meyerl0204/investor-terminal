...
with st.form(key="search_form", clear_on_submit=False):
    ...
    with _col_input:
        ticker_input = st.text_input("Enter Ticker (e.g., NVDA, TSLA, AAPL):", 
            value=st.session_state.get("last_ticker_input",""),
            label_visibility="collapsed").upper()
    with _col_btn:
        _search_clicked = st.form_submit_button("🔍 Analyze", type="primary", use_container_width=True)

# ── Stock grid helper ────────────────────────────────────────────────────────
# CSS for card-style buttons — injected once at module level
_CARD_BTN_CSS = """<style>
.card-btn-wrap > div > button {
...
.card-btn-wrap > div > button:hover {
    box-shadow: 0 4px 16px rgba(0,0,0,0.14) !important;
    border-color: #ccc !important;
}
.card-btn-wrap > div > button:focus {
    box-shadow: 0 2px 8px rgba(0,0,0,0.08) !important;
    outline: none !important;
}
</style>"""

def render_stock_grid(tickers):
    results = []
    for sym in tickers:
        try:
            ...
            if price and prev:
                ...
                if mktcap >= 1e12:   cap_str = f"${mktcap/1e12:.2f}T"
                elif mktcap >= 1e9:  cap_str = f"${mktcap/1e9:.1f}B"
                elif mktcap >= 1e6:  cap_str = f"${mktcap/1e6:.0f}M"
                else:                cap_str = "—"
                results.append({"sym": sym, "name": name, "price": price, "pct": pct, "cap": cap_str})
        except:
            pass
    if not results:
    ...
    # Inject CSS to make analyze buttons look seamless under cards
    st.markdown("""<style>
.analyze-btn > div > button {
    background: none !important; border: none !important;
    color: #2980b9 !important; font-size: 12px !important;
    font-weight: 600 !important; padding: 2px 0 10px 0 !important;
    cursor: pointer !important; width: 100% !important;
    text-align: left !important; box-shadow: none !important;
}
.analyze-btn > div > button:hover { text-decoration: underline !important; }
    ...
    for row in rows:
        ...
        for col, d in zip(cols, row):
            color = "#2ecc71" if d["pct"] >= 0 else "#e74c3c"
            bg    = "#eafaf1" if d["pct"] >= 0 else "#fdf0ef"
            arrow = "▲" if d["pct"] >= 0 else "▼"
            with col:
                st.markdown(f"""
<div style="background:white;border-radius:12px;padding:14px 16px 10px 16px;
box-shadow:0 2px 8px rgba(0,0,0,0.08);border:1px solid #e8e8e8;margin-bottom:2px;">
  <div style="font-size:15px;font-weight:800;color:#1a1a2e;">{d["sym"]}</div>
  <div style="font-size:11px;color:#888;margin-bottom:6px;white-space:nowrap;
  overflow:hidden;text-overflow:ellipsis;">{d["name"]}</div>
  <div style="font-size:16px;font-weight:700;color:#1a1a2e;">${d["price"]:,.2f}</div>
  <div style="display:inline-block;background:{bg};color:{color};border:1px solid {color};
  border-radius:5px;padding:1px 8px;font-size:12px;font-weight:700;margin-top:4px;">
  {arrow} {abs(d["pct"]):.2f}%</div>
  <div style="font-size:11px;color:#aaa;margin-top:4px;">{d["cap"]}</div>
</div>""", unsafe_allow_html=True)
                st.markdown('<div class="analyze-btn">', unsafe_allow_html=True)
                if st.button(f"Analyze {d['sym']} →", key=f"grid_{d['sym']}_{id(tickers)}"):
                    st.session_state["grid_click"] = d["sym"]
                st.markdown('</div>', unsafe_allow_html=True)

@st.cache_data(ttl=300)
def get_trending_tickers():
    try:
        result = yf.screen("most_actives", size=15)
        if result and "quotes" in result:
            syms = [q["symbol"] for q in result["quotes"] if "symbol" in q][:15]
            if syms:
                return syms
    except:
        pass
    # Fallback: fetch high-volume tickers and sort by volume
    try:
        fallback = ["NVDA","TSLA","AAPL","META","AMZN","MSFT","AMD","PLTR","GOOGL",
                    "NFLX","BAC","F","INTC","DIS","SOFI","GME","MSTR","RIVN","HOOD","SOFI"]
        vol_data = []
        for sym in fallback:
            try:
                fi = yf.Ticker(sym).fast_info
                vol_data.append((sym, fi.three_month_average_volume or 0))
            except:
                vol_data.append((sym, 0))
        vol_data.sort(key=lambda x: x[1], reverse=True)
        return [s for s, _ in vol_data[:15]]
    except:
        return ["NVDA","TSLA","AAPL","META","AMZN","MSFT","AMD","PLTR","GOOGL",
                "NFLX","BAC","F","INTC","DIS","SOFI"]

@st.cache_data(ttl=600)
def get_sp500_movers():
    # Top movers by % change from S&P 500
    sp500 = ["AAPL","MSFT","NVDA","AMZN","META","GOOGL","TSLA","BRK-B","JPM",
             "LLY","V","XOM","UNH","MA","AVGO","JNJ","HD","PG","MRK","ABBV",
             "COST","BAC","CRM","CVX","KO","PEP","WMT","TMO","CSCO","ADBE"]
    results = []
    for sym in sp500:
        try:
            fi = yf.Ticker(sym).fast_info
            ...
            if price and prev:
                pct = (price - prev) / prev * 100
                results.append((sym, abs(pct), pct))
        except: pass
    results.sort(key=lambda x: x[1], reverse=True)
    return [r[0] for r in results[:15]]

@st.cache_data(ttl=600)
def get_growth_tickers():
    return ["NVDA","META","AMZN","MSFT","GOOGL","CRM","NOW","SNOW","CRWD",
            "PANW","DDOG","NET","MELI","SHOP","SQ"]

@st.cache_data(ttl=600)
def get_dividend_tickers():
    return ["JNJ","KO","PEP","PG","MCD","T","VZ","O","ABBV","MRK",
            "CVX","XOM","IBM","MMM","WPC"]

@st.cache_data(ttl=600)
def get_ai_tech_tickers():
    return ["NVDA","MSFT","GOOGL","META","AMD","PLTR","AI","SMCI","ARM","INTC",
            "ORCL","IBM","TSMC","AVGO","QCOM"]

# ── Tabbed stock grid ─────────────────────────────────────────────────────────
st.markdown("<div style='margin-top:12px'></div>", unsafe_allow_html=True)
_tab1, _tab2, _tab3, _tab4, _tab5 = st.tabs([
    "🔥 Most Trending", "📊 S&P 500 Top Movers",
    "🚀 Growth", "💰 Dividend", "🤖 AI / Tech"
])

with _tab1:
    with st.spinner("Loading trending tickers..."):
        render_stock_grid(get_trending_tickers())

with _tab2:
    with st.spinner("Loading S&P 500 movers..."):
        render_stock_grid(get_sp500_movers())

with _tab3:
    render_stock_grid(get_growth_tickers())

with _tab4:
    render_stock_grid(get_dividend_tickers())

with _tab5:
    render_stock_grid(get_ai_tech_tickers())

st.markdown("<div style='margin:16px 0'></div>", unsafe_allow_html=True)

# Handle click from stock grid
if st.session_state.get("grid_click"):
    st.session_state["pending_ticker"] = st.session_state.pop("grid_click")
    st.rerun()

if _search_clicked and ticker_input:
    st.session_state["last_ticker_input"] = ticker_input
    st.session_state["pending_ticker"] = ticker_input

if st.session_state.get("pending_ticker"):
    _t = st.session_state.pop("pending_ticker")
    with st.spinner(f"Loading {_t}..."):
        try:
            _s = yf.Ticker(_t)
            st.session_state["stock_info"]       = _s.info
            st.session_state["hist_full"]        = _s.history(period="max")
            ...
            st.session_state["financials"]       = _s.financials
            st.session_state["quarterly_fin"]    = _s.quarterly_financials
            st.session_state["cashflow"]         = _s.cashflow
            st.session_state["quarterly_cf"]     = _s.quarterly_cashflow
            st.session_state["recommendations"]  = _s.recommendations
            st.session_state["news_list"]        = _s.news or []
            st.session_state["ticker_loaded"]    = _t
            st.session_state["analysis_done"]    = True
            st.session_state["price_range"]      = "1Y"
            st.rerun()
        except Exception as e:
            st.error(f"Error fetching data for {_t}: {e}")

# ── Render results if analysis has been run ──────────────────────────────────
st.markdown('<div id="analysis-top"></div>', unsafe_allow_html=True)
if st.session_state["analysis_done"]:
    # Scroll to analysis section
    st.markdown("""<script>
    setTimeout(function(){
        document.getElementById('analysis-top').scrollIntoView({behavior:'smooth'});
    }, 300);
