...
if st.session_state["analysis_done"]:
    ...
    if hist_full is not None and not hist_full.empty:
        ...
        # Compute pct for current range to set header emoji
        selected = st.session_state["price_range"]
        cur = all_ranges[selected]
        first_c = cur["prices"][0]
        last_c = cur["prices"][-1]
        pct = ((last_c - first_c) / first_c) * 100
        emoji = "📈" if pct >= 0 else "📉"
        arrow = "▲" if pct >= 0 else "▼"
        price_header.header(f"{emoji} Price History")
  ...
  const chart = new Chart(ctx, {{
    ...
    data: {{
      ...
      datasets: [{{
        ...
        pointRadius: 0,
        fill: true,
        backgroundColor: isUp ? "rgba(46,204,113,0.08)" : "rgba(231,76,60,0.08)",
        tension: 0.3
      }}]
    }},
    options: {{
