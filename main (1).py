...
with tab2:
    ...
    st.caption("Live Form 4 purchase filings pulled directly from SEC EDGAR. Refreshed hourly.")
    ...
    # ── Fetch SEC EDGAR full-text search for Form 4 purchase filings ──────────
    @st.cache_data(ttl=3600)
    def fetch_sec_insider_purchases():
        """
        Pulls recent Form 4 'P' (purchase) transactions from SEC EDGAR
        full-text search API — no API key, no rate limits beyond courtesy headers.
        Returns a DataFrame of purchase rows.
        """
        rows = []
        headers = {
            "User-Agent": "InvestorTerminal research@example.com",
            "Accept-Encoding": "gzip, deflate",
            "Accept": "application/json",
        }

        # EDGAR full-text search: Form 4, transaction code P = open-market purchase
        # We pull the 40 most recent filings
        url = (
            "https://efts.sec.gov/LATEST/search-index?q=%22transactionCode%22%3A%22P%22"
            "&dateRange=custom&startdt=2025-01-01&enddt=2099-01-01"
            "&forms=4&hits.hits._source=period_of_report,entity_name,file_date"
            "&hits.hits.total=true&hits.hits.highlight=false"
            "&_source=period_of_report,entity_name,file_date,period_of_report"
            "&hits.hits.highlight=false&hits.hits.total=true"
        )

        # Use EDGAR full-text search JSON endpoint
        search_url = (
            "https://efts.sec.gov/LATEST/search-index?"
            "q=%22transactionCode%3EP%3C%22&forms=4"
            "&dateRange=custom&startdt=2025-01-01"
            "&hits.hits._source=file_date,period_of_report,entity_name,file_num"
        )

        # Primary approach: EDGAR EFTS search API
        try:
            api_url = (
                "https://efts.sec.gov/LATEST/search-index?"
                "q=%22%3CtransactionCode%3EP%3C%22&forms=4"
                "&dateRange=custom&startdt=2025-12-01"
        ...
        # Fallback: EDGAR RSS feed for Form 4 filings (always works)
        rss_rows = []
        try:
            ...
            req2 = _urllib_req.Request(rss_url, headers={"User-Agent": "InvestorTerminal research@example.com"})
            with _urllib_req.urlopen(req2, timeout=12) as resp2:
                rss_xml = resp2.read().decode("utf-8", errors="replace")

            root = _ET.fromstring(rss_xml)
            ...
            for entry in entries:
                ...
                updated_txt = updated_el.text if updated_el is not None else ""
                link_href   = link_el.get("href", "") if link_el is not None else ""
                summary_txt = summary_el.text if summary_el is not None else ""

                # Parse company name from title like "4 - Apple Inc (0001234567) (Issuer)"
                company_match = _re.search(r"4\s*-\s*(.+?)\s*\(", title_txt)
                company = company_match.group(1).strip() if company_match else title_txt[:40]

                # Parse filing date
                try:
                    file_date = pd.to_datetime(updated_txt).date()
                except Exception:
                    file_date = None
        ...
        # ── For each RSS filing, fetch the actual XML to get transaction details ──
        # We cap at 60 filings and only parse purchases (transactionCode = P)
        for r in rss_rows[:60]:
            try:
                # Build the filing index URL from the atom link
                link = r["_link"]
                # Convert viewer link to index page
                # e.g. https://www.sec.gov/Archives/edgar/data/CIK/ACC/doc.htm
                # → fetch the filing index to find the XML
                index_url = link.replace("-index.htm", "-index.json")
                if not index_url.endswith("-index.json"):
                    # Try direct construction
                    index_url = link.rsplit("/", 1)[0] + "/" + link.rsplit("/", 1)[-1].replace(".htm", "-index.json")

                idx_req = _urllib_req.Request(index_url, headers={"User-Agent": "InvestorTerminal research@example.com"})
                with _urllib_req.urlopen(idx_req, timeout=8) as idx_resp:
                    idx_data = _json.loads(idx_resp.read())
                ...
                xml_req = _urllib_req.Request(xml_url, headers={"User-Agent": "InvestorTerminal research@example.com"})
                with _urllib_req.urlopen(xml_req, timeout=8) as xml_resp:
                    xml_content = xml_resp.read().decode("utf-8", errors="replace")

                form4 = _ET.fromstring(xml_content)

                # Issuer info
                issuer     = form4.find(".//issuer")
                ticker_sym = issuer.findtext("issuerTradingSymbol", "").strip().upper() if issuer is not None else ""
                co_name    = issuer.findtext("issuerName", r["_company"]).strip() if issuer is not None else r["_company"]

                # Reporting owner
                owner      = form4.find(".//reportingOwner")
                owner_name = ""
                owner_title = ""
                if owner is not None:
                    owner_name  = owner.findtext(".//rptOwnerName", "").strip()
                    owner_title = owner.findtext(".//officerTitle", "")
                    if not owner_title:
                        is_officer  = owner.findtext(".//isOfficer", "0")
                        is_director = owner.findtext(".//isDirector", "0")
                        is_10pct    = owner.findtext(".//isTenPercentOwner", "0")
                        if is_officer == "1":   owner_title = "Officer"
                        elif is_director == "1": owner_title = "Director"
                        elif is_10pct == "1":    owner_title = "10% Owner"
                        else:                    owner_title = "Insider"

                # Non-derivative transactions
                for txn in form4.findall(".//nonDerivativeTransaction"):
                    ...
                    try:
                        shares = int(float(shares_el.text)) if shares_el is not None else None
                    ...
                    try:
                        ...
                        price = None
                    try:
                        txn_date = pd.to_datetime(date_el.text).date() if date_el is not None else r["_date"]
                    except Exception:
                        txn_date = r["_date"]

                    value = shares * price if shares and price else None

                    # Get market cap for the ticker
                    mktcap = None
                    if ticker_sym:
                        try:
                            mktcap = yf.Ticker(ticker_sym).fast_info.market_cap
                        except Exception:
                            pass

                    rows.append({
                        "Ticker":     ticker_sym or "—",
                        "Company":    co_name,
                        "Insider":    owner_name or "—",
        ...
        if not df.empty:
            # Drop rows with no ticker or tiny noise trades < $1k
            df = df[df["Ticker"] != "—"]
            df = df[df["Value ($)"].fillna(0) > 1000]
            df = df.drop_duplicates(subset=["Ticker", "Insider", "Date", "Shares"])
            df = df.sort_values("Date", ascending=False).reset_index(drop=True)
        return df

    with st.spinner("Fetching live Form 4 filings from SEC EDGAR..."):
        insider_df = fetch_sec_insider_purchases()

    if insider_df.empty:
        st.warning(
            "No purchase filings parsed from SEC EDGAR this session. "
            "This can happen if EDGAR is slow or all recent Form 4s were non-purchase transactions. "
            "Try hitting **R** to rerun, or check [EDGAR directly](https://www.sec.gov/cgi-bin/browse-edgar?action=getcurrent&type=4&dateb=&owner=include&count=40)."
        )
    else:
        ...
        with sfc2:
            min_val = st.number_input("Min purchase value ($)", min_value=0, value=0, step=10000,
                                      help="Hide small open-market buys", key="insider_minval")
        with sfc3:
        ...
        num_buys    = len(display_df)
        unique_cos  = display_df["Ticker"].nunique()

        sm1, sm2, sm3 = st.columns(3)
        for _col, _label, _val in [
            (sm1, "Total $ Purchased",  _fmt_val(total_value) if total_value > 0 else "N/A"),
            (sm2, "Purchase Events",    str(num_buys)),
            (sm3, "Companies",          str(unique_cos)),
        ]:
            with _col:
                st.markdown(
        ...
        for chunk in chunks:
            ...
            for idx, (_, r) in enumerate(chunk.iterrows()):
                ...
                shares_str  = f"{int(r['Shares']):,}" if r["Shares"] else "—"
                insider_str = str(r["Insider"])[:28] + ("…" if len(str(r["Insider"])) > 28 else "")
                title_str   = str(r["Title"])[:30]   + ("…" if len(str(r["Title"]))   > 30 else "")
                sec_link    = r.get("SEC Link", "")
                ticker_disp = r["Ticker"] if r["Ticker"] != "—" else "?"
                link_html   = f'<a href="{sec_link}" target="_blank" style="font-size:10px;color:#999;text-decoration:none;">📄 SEC Filing</a>' if sec_link else ""
                    ...
                    st.markdown(
                        ...
                        border-top:3px solid #2ecc71;">
                          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">
                            <span style="font-size:18px;font-weight:800;color:#1a1a2e;">{ticker_disp}</span>
                            <span style="font-size:11px;background:#eafaf1;color:#1a9e5c;border-radius:6px;
                            padding:3px 8px;font-weight:600;">{date_str}</span>
                          </div>
                          <div style="font-size:12px;color:#666;margin-bottom:8px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{r['Company']}</div>
                          <div style="font-size:24px;font-weight:700;color:#2ecc71;margin-bottom:2px;">{val_str}</div>
                          <div style="font-size:12px;color:#888;margin-bottom:8px;">{shares_str} shares</div>
                          ...
                          <div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px;">
                            <span style="font-size:11px;color:#aaa;">Mkt Cap: {mktcap_str}</span>
                            {link_html}
                          </div>
                        </div>""",
                        unsafe_allow_html=True
