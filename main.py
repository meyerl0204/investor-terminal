        ...
        try:
            ...
            if news_list:
                for news in news_list[:5]:
                    with st.expander(f"{news.get('publisher', 'News')} - {news.get('title', 'Article')}"):
                        st.write(f"**Link:** [Read Article]({news.get('link', '#')})")
                        st.write("Nerd Note: Scan for phrases like 'Quarterly Outlook' or 'CEO Comments'.")
            else:
