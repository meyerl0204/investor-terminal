...
if st.session_state["analysis_done"]:
    ...
    # ── DEBUG (remove later) ──
    with st.expander("🔧 Debug: Raw yfinance fields"):
        useful = {k: v for k, v in info.items() if v is not None and v != "" and v != 0}
        st.json(useful)
...
<script>
  ...
  function switchRange(range) {{
    ...
    # Debt/Equity — fallback: totalDebt / stockholdersEquity * 100
    de = info.get("debtToEquity")
    if de is None and total_debt and equity and equity != 0:
        de = round((total_debt / abs(equity)) * 100, 1)

    # ROE / ROA
    roe = info.get("returnOnEquity")
    roa = info.get("returnOnAssets")
    # Fallback ROE: net income / equity
    if roe is None and net_income and equity and equity != 0:
        roe = net_income / abs(equity)
    # Fallback ROA: net income / total assets (use equity+debt as proxy)
    ...
    mg_val = f"{margin*100:.1f}%" if margin != 0 else "N/A"
    ...
    ps_val = f"{ps_ratio:.1f}x" if ps_ratio is not None else "N/A"
    ...
    eg_val = f"{earn_growth*100:.1f}%" if earn_growth is not None else "N/A"
    ...
    if pfcf is not None and pfcf > 0:
        if pfcf < 15:    pfcf_b = badge("Cheap",     "#27ae60", "#eafaf1")
...
.card{background:#f8f9fa;border-radius:10px;padding:12px 16px;flex:1;min-width:140px;border-top:3px solid #e0e0e0;}
