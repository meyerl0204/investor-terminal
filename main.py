...
def get_popular_stocks():
    tickers = ["NVDA","AAPL","MSFT","AMZN","META","GOOGL","TSLA","JPM",
               "BRK-B","V","MA","XOM","WMT","LLY","AVGO"]
    results = []
    for sym in tickers:
        try:
            ...
            if price and prev:
                ...
                else:                cap_str = f"${mktcap/1e6:.0f}M"
                results.append({"sym": sym, "name": name, "price": price, "pct": pct, "cap": cap_str})
        except:
            pass
    return results

_popular = get_popular_stocks()

if _popular:
    st.markdown(
        '<div style="font-size:13px;color:#888;font-weight:600;'
        'text-transform:uppercase;letter-spacing:.6px;margin:8px 0 10px 0;">'
        '📈 Popular Stocks</div>',
        unsafe_allow_html=True
    )
    # 5 cards per row, 3 rows = 15 stocks
    rows = [_popular[i:i+5] for i in range(0, len(_popular), 5)]
    for row in rows:
        cols = st.columns(5)
        for col, stock_data in zip(cols, row):
            color  = "#2ecc71" if stock_data["pct"] >= 0 else "#e74c3c"
            bg     = "#eafaf1" if stock_data["pct"] >= 0 else "#fdf0ef"
            arrow  = "▲" if stock_data["pct"] >= 0 else "▼"
            with col:
                st.markdown(f"""
...
onmouseout="this.style.boxShadow='none'">
  <div style="font-size:15px;font-weight:800;color:#1a1a2e;">{stock_data["sym"]}</div>
  <div style="font-size:11px;color:#888;margin-bottom:6px;white-space:nowrap;
  overflow:hidden;text-overflow:ellipsis;">{stock_data["name"]}</div>
  <div style="font-size:16px;font-weight:700;color:#1a1a2e;">${stock_data["price"]:,.2f}</div>
  ...
  {arrow} {abs(stock_data["pct"]):.2f}%</div>
  <div style="font-size:11px;color:#aaa;margin-top:4px;">{stock_data["cap"]}</div>
