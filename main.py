...
rows.forEach((r, i) => {{
  ...
  tbody.innerHTML += `
    ...
    try:
        ...
        # Try session state first, fallback to fresh fetch with shorter period
        if spy_hist_raw is None or spy_hist_raw.empty:
            try:
                spy_hist_raw = yf.Ticker("SPY").history(period="10y")
            except:
                spy_hist_raw = None
        ...
        if spy_hist_raw is None or spy_hist_raw.empty:
            ...
            spy_hist = spy_hist_raw[["Close"]].copy()
        ...
        today = pd.Timestamp.today().normalize()
        ranges = {"1M": 30, "3M": 90, "6M": 180, "1Y": 365, "3Y": 1095, "5Y": 1825, "ALL": None}
        ...
        for label, days in ranges.items():
            if days:
                ...
                start = stock_hist.index.min()

            s = stock_hist[stock_hist.index >= start]["Close"]
            m = spy_hist[spy_hist.index >= start]["Close"]
            ...
            all_ranges[label] = {
                "dates": dates, "stock": s_norm, "spy": m_norm,
