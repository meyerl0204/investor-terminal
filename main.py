...
with tab1:
    ...
    st.sidebar.divider()
    st.sidebar.header("📊 Extended Financial History")
    simfin_api_key = st.sidebar.text_input("Simfin API Key", type="password",
        help="Free key at app.simfin.com — unlocks 7 years of financial history")
        ...
        with st.spinner("Fetching data..."):
            ...
            try:
                ...
                if simfin_api_key:
                    ...
                    # Debug: show raw metric names so we can verify mapping
                    with st.expander("🔍 Simfin debug — raw metric names (can remove later)"):
                        for k, v in sf_data.items():
                            if v is not None and not v.empty:
                                st.write(f"**{k}** — {list(v.index)}")
                            else:
                                st.write(f"**{k}** — empty")
                else:
            ...
            tooltip: {{
              callbacks: {{
                label: c => {{
                  ...
                  if (ds.isShares && ds.suffix === "B") return Math.abs(v).toFixed(2) + "B shares";
            ...
            y: {{
              ticks: {{
                ...
                callback: v => {{
                  ...
                  if (ds.isEps) return "$" + v.toFixed(2);
                  if (ds.isShares && ds.suffix === "B") return v.toFixed(1) + "B";
