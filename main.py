...
# S&P 500 historical data embedded directly (no CSV file needed)
...
anthropic_api_key = st.sidebar.text_input(
    "Anthropic API Key",
    type="password",
    help="Enter your Anthropic API key to enable AI news summaries. Get one free at console.anthropic.com"
)

def get_ai_summary(ticker, headline, publisher, api_key):
    """Generate an AI summary for a news headline."""
    try:
        ...
        message = client.messages.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": f"""You are a financial analyst assistant. Based on this news headline about {ticker}, provide a concise 2-3 sentence investor-focused summary of what this likely means for the stock. Focus on potential impact on price, earnings, or investor sentiment.
    ...
    with st.spinner("Fetching data..."):
        try:
            ...
            if not anthropic_api_key:
                st.info("💡 Enter your Anthropic API key in the sidebar to get AI-powered summaries for each article.")
                    ...
                    with st.expander(f"📰 {publisher} — {headline}"):
                        ...
                        if anthropic_api_key:
                            with st.spinner("🤖 Generating AI summary..."):
                                summary = get_ai_summary(ticker, headline, publisher, anthropic_api_key)
                            st.markdown("**🤖 AI Investor Summary:**")
                            st.info(summary)
                        else:
                            st.caption("🔒 Add your API key in the sidebar to unlock AI summaries.")
