...
with _col_input:
    ticker_input = st.text_input("Enter Ticker (e.g., NVDA, TSLA, AAPL):", "", label_visibility="collapsed").upper()
with _col_btn:
    _search_clicked = st.button("🔍 Analyze", type="primary", use_container_width=True)
...
if "grid_click" in st.session_state and st.session_state["grid_click"]:
    _clicked = st.session_state["grid_click"]
    st.session_state["grid_click"] = None
    with st.spinner(f"Loading {_clicked}..."):
        try:
            stock = yf.Ticker(_clicked)
            ...
            st.session_state["ticker_loaded"]   = _clicked
