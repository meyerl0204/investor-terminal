...
if st.session_state["analysis_done"]:
    ...
    def _series_to_chart(series, title, color, is_shares=False, flip_neg=False):
    ...
    # Quarterly charts
    quarterly_charts = [
        _series_to_chart(_get_fin(quarterly_fin, "Total Revenue"), "Revenue (Q)", "#2ecc71"),
        _series_to_chart(_get_fin(quarterly_fin, "Net Income"), "Net Income (Q)", "#4fc3f7"),
        _series_to_chart(_get_fin(quarterly_fin, "Basic EPS","Diluted EPS"), "EPS (Q)", "#9b59b6"),
        _series_to_chart(_get_fin(quarterly_cf, "Free Cash Flow"), "FCF (Q)", "#e67e22"),
        _series_to_chart(_get_fin(quarterly_fin, "Gross Profit"), "Gross Profit (Q)", "#1abc9c"),
        _series_to_chart(_get_fin(quarterly_fin, "Operating Income"), "Op. Income (Q)", "#e74c3c"),
        _series_to_chart(_get_fin(q_balance_sheet, "Ordinary Shares Number","Share Issued","Common Stock"), "Shares Out. (Q)", "#e91e8c", is_shares=True),
        _series_to_chart(_get_fin(quarterly_cf, "Capital Expenditure","Purchase Of Property Plant And Equipment","Capital Expenditures"), "CapEx (Q)", "#8e44ad", flip_neg=True),
    ]
    ...
    if q_rpo is not None and q_rpo.abs().max() > 0:
        quarterly_charts.append(_series_to_chart(q_rpo, "RPO / Def. Rev (Q)", "#16a085"))
