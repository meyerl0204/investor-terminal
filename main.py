...
with tab2:
    ...
    st.caption("Insider purchases filed with the SEC via Form 4. Data sourced from yfinance / SEC EDGAR.")

    INSIDER_WATCHLIST = [
        "AAPL","MSFT","NVDA","GOOGL","AMZN","META","TSLA","AVGO","JPM","V",
        "UNH","XOM","LLY","JNJ","MA","PG","HD","COST","ABBV","MRK",
        "BAC","CVX","WMT","KO","NFLX","CRM","AMD","ORCL","TMO","ADBE",
        "NOW","UBER","PLTR","SMCI","ARM","MSTR","COIN","HOOD","RKLB","LUNR",
    ]
    ...
    def fetch_insider_data(tickers):
        ...
        for sym in tickers:
            try:
                ...
                company = t.info.get("shortName") or sym
                ...
                # Keep only purchases (positive share counts)
                buy_col = None
                for candidate in ["Shares", "shares", "Value"]:
                    if candidate in it.columns:
                        buy_col = candidate
                        break

                # Filter for buys: "Transaction" column contains "Buy" or "Purchase"
                txn_col = None
                for candidate in ["Transaction", "transaction", "Type", "type"]:
                    if candidate in it.columns:
                        txn_col = candidate
                        break

                if txn_col:
                    it = it[it[txn_col].astype(str).str.contains("Buy|Purchase|buy|purchase", na=False)]
                ...
                # Value column
                val_col = None
                for candidate in ["Value", "value", "Amount", "amount"]:
                    if candidate in it.columns:
                        val_col = candidate
                        break
                ...
                # Date column
                date_col = None
                for candidate in ["Start Date", "startDate", "Date", "date", "Filing Date", "filingDate"]:
                    if candidate in it.columns:
                        date_col = candidate
                        break
                ...
                # Insider name column
                name_col = None
                for candidate in ["Insider", "insider", "Name", "Filer Name", "filerName"]:
                    if candidate in it.columns:
                        name_col = candidate
                        break
                ...
                title_col = None
                for candidate in ["Position", "position", "Title", "title", "Relationship", "relationship"]:
                    if candidate in it.columns:
                        title_col = candidate
                        break

                for _, row in it.iterrows():
                    val = row[val_col] if val_col and val_col in row else None
                    try:
                        val = float(val) if val is not None else None
                    except (TypeError, ValueError):
                        val = None

                    shares = row[buy_col] if buy_col and buy_col in row else None
                    try:
                    ...
                    rows.append({
                        "Ticker": sym,
                        "Company": company,
                        "Insider": row[name_col] if name_col and name_col in row else "—",
                        "Title": row[title_col] if title_col and title_col in row else "—",
                        "Date": date_val,
                        "Shares": shares,
                        "Value ($)": val,
                        "Market Cap": mktcap,
    ...
    with st.spinner("Loading insider purchase data across S&P 500 / notable names..."):
        insider_df = fetch_insider_data(tuple(INSIDER_WATCHLIST))

    if insider_df.empty:
        st.warning("No recent insider purchase data found. yfinance may be rate-limiting — try again in a moment.")
    else:
        # ── Sort controls ──────────────────────────────────────────────────
        ...
        with sort_col2:
            min_val = st.number_input("Min purchase value ($)", min_value=0, value=0, step=10000,
                                      help="Filter out small open-market purchases")
        with sort_col3:
            search_ticker = st.text_input("Filter by ticker", "").upper().strip()
        ...
        if sort_by == "Ticker A → Z":
            ...
            display_df = display_df.sort_values("Market Cap", ascending=False)
        else:
        ...
        for col, label, val in [
            (sm1, "Total $ Purchased", f"${total_value:,.0f}" if pd.notna(total_value) and total_value > 0 else "N/A"),
            ...
            with col:
                st.markdown(
                    ...
                    <div style="font-size:11px;color:#999;text-transform:uppercase;letter-spacing:.6px;">{label}</div>
        ...
        CARDS_PER_ROW = 3
        rows_list = [display_df.iloc[i:i+CARDS_PER_ROW] for i in range(0, len(display_df), CARDS_PER_ROW)]

        for chunk in rows_list:
            cols = st.columns(CARDS_PER_ROW)
            for idx, (_, r) in enumerate(chunk.iterrows()):
                date_str   = str(r["Date"]) if r["Date"] else "—"
                val_str    = fmt_val(r["Value ($)"])
                mktcap_str = fmt_mktcap(r["Market Cap"])
                shares_str = f"{r['Shares']:,}" if r["Shares"] else "—"
                insider    = str(r["Insider"])[:28] + ("…" if len(str(r["Insider"])) > 28 else "")
                title      = str(r["Title"])[:30] + ("…" if len(str(r["Title"])) > 30 else "")
                ...
                with cols[idx]:
                    st.markdown(
                        ...
                        border-top:3px solid #2ecc71;">
                          ...
                          <div style="font-size:12px;color:#666;margin-bottom:10px;">{r['Company']}</div>
                          <div style="font-size:22px;font-weight:700;color:#2ecc71;margin-bottom:4px;">{val_str}</div>
                          ...
                          <div style="font-size:12px;color:#444;font-weight:600;">{insider}</div>
