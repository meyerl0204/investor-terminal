...
rows.forEach((r, i) => {{
  ...
  tbody.innerHTML += `
    ...
    try:
        ...
        # Get SPY — session state first, live fallback
        spy_raw = spy_hist_raw
        if spy_raw is None or spy_raw.empty:
            spy_raw = yf.Ticker("SPY").history(period="10y")

        if spy_raw is None or spy_raw.empty:
            st.info("SPY data temporarily unavailable — try regenerating.")
            raise StopIteration

        # Normalize indexes — strip timezone
        spy_close = spy_raw[["Close"]].copy()
        spy_close.index = pd.to_datetime(spy_close.index).tz_localize(None).normalize()

        stock_close = hist_full[["Close"]].copy()
        stock_close.index = pd.to_datetime(stock_close.index).tz_localize(None).normalize()

        # Deduplicate indexes (keep last)
        spy_close   = spy_close[~spy_close.index.duplicated(keep='last')]
        stock_close = stock_close[~stock_close.index.duplicated(keep='last')]
        ...
        for label, days in range_days.items():
            start = today - pd.Timedelta(days=days) if days else stock_close.index.min()
            s = stock_close[stock_close.index >= start]["Close"]
            m = spy_close[spy_close.index >= start]["Close"]
            ...
            s = s.loc[common]
            m = m.loc[common]
            s_norm = (s / s.iloc[0] * 100).round(2).tolist()
            m_norm = (m / m.iloc[0] * 100).round(2).tolist()
            dates  = [str(d.date()) for d in common]
            s_ret  = round((s.iloc[-1] / s.iloc[0] - 1) * 100, 2)
            m_ret  = round((m.iloc[-1] / m.iloc[0] - 1) * 100, 2)
