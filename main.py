...
def make_kpi_chart(data, title, y_label, color="#4c9be8", format_billions=True):
    ...
    ).properties(height=250, padding={"top": 10, "bottom": 10})
    return chart, title
            ...
            try:
                ...
                with col1:
                    if financials is not None and 'Total Revenue' in financials.index:
                        rev = financials.loc['Total Revenue'].sort_index()
                        c, t = make_kpi_chart(rev, "Annual Revenue", "Revenue", "#2ecc71")
                        if c:
                            st.subheader(t)
                            st.altair_chart(c, use_container_width=True)
                with col2:
                    if financials is not None and 'Net Income' in financials.index:
                        ni = financials.loc['Net Income'].sort_index()
                        c, t = make_kpi_chart(ni, "Net Income", "Net Income", "#3498db")
                        if c:
                            st.subheader(t)
                            st.altair_chart(c, use_container_width=True)

                # ── Row 2: EPS & Free Cash Flow ──
                col3, col4 = st.columns(2)
                with col3:
                    if financials is not None and 'Basic EPS' in financials.index:
                        eps = financials.loc['Basic EPS'].sort_index()
                        c, t = make_kpi_chart(eps, "Earnings Per Share (EPS)", "EPS ($)", "#9b59b6", format_billions=False)
                        if c:
                            st.subheader(t)
                            st.altair_chart(c, use_container_width=True)
                    elif financials is not None and 'Diluted EPS' in financials.index:
                        eps = financials.loc['Diluted EPS'].sort_index()
                        c, t = make_kpi_chart(eps, "Diluted EPS", "EPS ($)", "#9b59b6", format_billions=False)
                        if c:
                            st.subheader(t)
                            st.altair_chart(c, use_container_width=True)
                with col4:
                    if cashflow is not None and 'Free Cash Flow' in cashflow.index:
                        fcf = cashflow.loc['Free Cash Flow'].sort_index()
                        c, t = make_kpi_chart(fcf, "Free Cash Flow", "FCF", "#e67e22")
                        if c:
                            st.subheader(t)
                            st.altair_chart(c, use_container_width=True)

                # ── Row 3: Gross Profit & Operating Income ──
                col5, col6 = st.columns(2)
                with col5:
                    if financials is not None and 'Gross Profit' in financials.index:
                        gp = financials.loc['Gross Profit'].sort_index()
                        c, t = make_kpi_chart(gp, "Gross Profit", "Gross Profit", "#1abc9c")
                        if c:
                            st.subheader(t)
                            st.altair_chart(c, use_container_width=True)
                with col6:
                    if financials is not None and 'Operating Income' in financials.index:
                        oi = financials.loc['Operating Income'].sort_index()
                        c, t = make_kpi_chart(oi, "Operating Income", "Operating Income", "#e74c3c")
                        if c:
                            st.subheader(t)
                            st.altair_chart(c, use_container_width=True)
