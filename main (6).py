...
<div class="grid">""" + cards_html + """</div></body></html>"""
    ...
    # ── EPS History Chart ────────────────────────────────────────────────────
    st.header("📊 Earnings Per Share History")
    try:
        eps_q = None
        if quarterly_fin is not None and 'Basic EPS' in quarterly_fin.index:
            eps_q = quarterly_fin.loc['Basic EPS'].sort_index()
        elif quarterly_fin is not None and 'Diluted EPS' in quarterly_fin.index:
            eps_q = quarterly_fin.loc['Diluted EPS'].sort_index()
        if eps_q is not None and not eps_q.empty:
            ch, bg = bar_chart(eps_q, "EPS ($)", "#9b59b6", format_billions=False, quarterly=True)
            if ch:
                st.altair_chart(ch, use_container_width=True)
                render_growth_badges(bg)
        else:
