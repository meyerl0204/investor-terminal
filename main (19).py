...
def render_stock_grid(tickers):
    ...
    # Inject CSS once to style the click buttons to be invisible overlays
    st.markdown("""<style>
...
margin-bottom:4px;box-shadow:0 2px 8px rgba(0,0,0,0.08);
border:1px solid #e8e8e8;position:relative;">
  ...
  <div style="font-size:11px;color:#aaa;margin-top:4px;">{d["cap"]}</div>
                ...
                # Invisible button that triggers the analysis
                if st.button(f"▶ {d['sym']}", key=f"grid_{d['sym']}_{id(tickers)}"):
