        ...
        with c6:
            if financials is not None and 'Operating Income' in financials.index:
                ...
                if ch:
                    ...
                    st.altair_chart(ch, use_container_width=True)
                    render_growth_badges(bg)
        # Shares Outstanding + CapEx (annual)
        c7, c8 = st.columns(2)
        with c7:
            shares_data = None
            ...
            for capex_key in ['Capital Expenditure', 'Purchase Of Property Plant And Equipment',
                ...
                try:
                    if cashflow is not None and capex_key in cashflow.index:
                        capex_data = cashflow.loc[capex_key].sort_index()
                        if capex_data.mean() < 0:
                            capex_data = capex_data * -1
                        break
                except: pass
            if capex_data is not None and not capex_data.empty:
                ch, bg = bar_chart(capex_data, "CapEx", "#8e44ad")
                if ch:
                    st.subheader("Capital Expenditures")
                    st.altair_chart(ch, use_container_width=True)
                    render_growth_badges(bg)

        # RPO — only shown if data exists (SaaS/cloud companies like MSFT, ORCL, CRM, SNOW)
        rpo_data = None
        for rpo_key in ['Remaining Performance Obligation', 'DeferredRevenue',
                        'Deferred Revenue', 'ContractWithCustomerLiability']:
            try:
                bs = stock.balance_sheet
                if bs is not None and rpo_key in bs.index:
                    rpo_data = bs.loc[rpo_key].sort_index()
                    break
            except: pass
        if rpo_data is not None and not rpo_data.empty and rpo_data.abs().max() > 0:
            c_rpo1, c_rpo2 = st.columns(2)
            with c_rpo1:
                ch, bg = bar_chart(rpo_data, "RPO / Deferred Rev", "#16a085")
                if ch:
            ...
            for capex_key in ['Capital Expenditure', 'Purchase Of Property Plant And Equipment',
                ...
                try:
                    if quarterly_cf is not None and capex_key in quarterly_cf.index:
                        q_capex_data = quarterly_cf.loc[capex_key].sort_index()
                        if q_capex_data.mean() < 0:
                            q_capex_data = q_capex_data * -1
                        break
                except: pass
            if q_capex_data is not None and not q_capex_data.empty:
                ch, bg = bar_chart(q_capex_data, "CapEx", "#8e44ad", quarterly=True)
                if ch:
        ...
        for rpo_key in ['Remaining Performance Obligation', 'DeferredRevenue',
            ...
            try:
                ...
                if qbs is not None and rpo_key in qbs.index:
                    q_rpo_data = qbs.loc[rpo_key].sort_index()
                    break
            except: pass
        if q_rpo_data is not None and not q_rpo_data.empty and q_rpo_data.abs().max() > 0:
            c_rpo1, c_rpo2 = st.columns(2)
            with c_rpo1:
                ch, bg = bar_chart(q_rpo_data, "RPO / Deferred Rev", "#16a085", quarterly=True)
                if ch:
...
rows.forEach((r, i) => {{
  ...
  tbody.innerHTML += `
    ...
    try:
        ...
        # Get yearly returns from price history
        stock_hist = hist_full[["Close"]].copy()
        stock_hist.index = pd.to_datetime(stock_hist.index).tz_localize(None)

        # Download SPY yearly data
        spy_raw = yf.download("SPY", period="max", progress=False, auto_adjust=True)
        if isinstance(spy_raw.columns, pd.MultiIndex):
            spy_raw.columns = spy_raw.columns.get_level_values(0)
        spy_hist = spy_raw[["Close"]].copy()
        spy_hist.index = pd.to_datetime(spy_hist.index).tz_localize(None)

        def yearly_returns(df, col="Close"):
            df = df.copy()
            df["year"] = df.index.year
            yearly = {}
            for yr, grp in df.groupby("year"):
                grp = grp.sort_index()
                if len(grp) >= 20:  # at least ~1 month of data
                    ret = (grp[col].iloc[-1] / grp[col].iloc[0] - 1) * 100
                    yearly[int(yr)] = round(ret, 1)
            return yearly

        stock_yrs = yearly_returns(stock_hist)
        spy_yrs   = yearly_returns(spy_hist)

        # Common years only
        common_yrs = sorted(set(stock_yrs.keys()) & set(spy_yrs.keys()))

        if len(common_yrs) < 2:
            st.info("Not enough yearly data to build comparison.")
        else:
            rows = []
            for yr in common_yrs:
                ...
                m = spy_yrs[yr]
                diff = round(s - m, 1)
                rows.append({"year": yr, "stock": s, "spy": m, "diff": diff})

            ticker_label = loaded_ticker
            rows_json = json.dumps(rows)

            # Summary stats
            beat_count = sum(1 for r in rows if r["stock"] > r["spy"])
            avg_stock  = round(sum(r["stock"] for r in rows) / len(rows), 1)
            avg_spy    = round(sum(r["spy"]   for r in rows) / len(rows), 1)
            avg_diff   = round(avg_stock - avg_spy, 1)

            perf_html = f"""<!DOCTYPE html><html><head>
<style>
body{{font-family:-apple-system,sans-serif;margin:0;padding:0;}}
.summary{{display:flex;gap:12px;margin-bottom:16px;flex-wrap:wrap;}}
.pill{{background:#f0f0f0;border-radius:8px;padding:8px 16px;font-size:13px;font-weight:600;color:#444;}}
.pill span{{font-weight:800;}}
.pos{{color:#27ae60;}}.neg{{color:#e74c3c;}}
table{{width:100%;border-collapse:collapse;font-size:13px;}}
th{{background:#f8f9fa;padding:8px 12px;text-align:right;font-weight:700;color:#555;
    border-bottom:2px solid #e0e0e0;font-size:11px;text-transform:uppercase;letter-spacing:.5px;}}
th:first-child{{text-align:left;}}
td{{padding:7px 12px;text-align:right;border-bottom:1px solid #f0f0f0;font-size:13px;}}
td:first-child{{text-align:left;font-weight:700;color:#444;}}
tr:last-child td{{border-bottom:none;}}
...
.cur-yr td{{background:#fffbf0 !important;font-weight:600;}}
</style></head><body>
<div class="summary">
  <div class="pill">📅 Years tracked: <span>{len(rows)}</span></div>
  <div class="pill">📈 {ticker_label} avg/yr: <span class="{'pos' if avg_stock>=0 else 'neg'}">{'+' if avg_stock>=0 else ''}{avg_stock}%</span></div>
  <div class="pill">📊 S&P 500 avg/yr: <span class="{'pos' if avg_spy>=0 else 'neg'}">{'+' if avg_spy>=0 else ''}{avg_spy}%</span></div>
  <div class="pill">🏆 Beat market: <span class="{'pos' if beat_count>len(rows)/2 else 'neg'}">{beat_count}/{len(rows)} yrs</span></div>
  <div class="pill">⚡ Avg edge: <span class="{'pos' if avg_diff>=0 else 'neg'}">{'+' if avg_diff>=0 else ''}{avg_diff}%/yr</span></div>
</div>
<table>
  <thead>
    <tr>
...
<script>
const rows = {rows_json};
const maxAbs = Math.max(...rows.map(r => Math.max(Math.abs(r.stock), Math.abs(r.spy))));
...
[...rows].reverse().forEach(r => {{
  ...
  const sColor = r.stock >= 0 ? "#27ae60" : "#e74c3c";
  const mColor = r.spy   >= 0 ? "#27ae60" : "#e74c3c";
  const dColor = r.diff  >= 0 ? "#27ae60" : "#e74c3c";
  const dArrow = r.diff  >= 0 ? "▲" : "▼";
  const sW = Math.round(Math.abs(r.stock) / maxAbs * 80);
  const mW = Math.round(Math.abs(r.spy)   / maxAbs * 80);

  tbody.innerHTML += `<tr class="${{isCur ? 'cur-yr' : ''}}">
    <td>${{r.year}}${{isCur ? ' <span style="font-size:10px;color:#f39c12">●YTD</span>' : ''}}</td>
    <td style="color:${{sColor}};font-weight:700">${{r.stock >= 0 ? '+' : ''}}${{r.stock}}%</td>
    <td style="color:${{mColor}};font-weight:700">${{r.spy   >= 0 ? '+' : ''}}${{r.spy}}%</td>
    <td style="color:${{dColor}};font-weight:700">${{dArrow}} ${{Math.abs(r.diff)}}%</td>
    <td>
      <div class="bar-wrap">
        <span style="font-size:10px;color:#2980b9;width:28px;text-align:right">${{r.stock>=0?'+':''}}${{r.stock}}%</span>
        <div class="bar" style="width:${{sW}}px;background:#2980b9;opacity:0.85"></div>
        <span style="font-size:10px;color:#888">|</span>
        <div class="bar" style="width:${{mW}}px;background:#e67e22;opacity:0.85"></div>
        <span style="font-size:10px;color:#e67e22;width:28px">${{r.spy>=0?'+':''}}${{r.spy}}%</span>
      </div>
    </td>
  </tr>`;
}});
            ...
            components.html(perf_html, height=min(80 + len(rows) * 36, 900), scrolling=True)

    except Exception as e:
        st.info(f"Performance comparison unavailable: {e}")
    st.divider()
