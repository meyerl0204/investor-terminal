...
<script>
  ...
  function switchRange(range) {{
    ...
    # Dividend Yield — yfinance returns as raw decimal (0.0002 = 0.02%)
    # Normalize: if value > 0.20 it was already given as a percentage, divide by 100
    if div_yield and div_yield > 0:
        if div_yield > 0.20:
            div_yield = div_yield / 100
        dy_pct = div_yield * 100  # now in display % e.g. 0.02 for NVDA
