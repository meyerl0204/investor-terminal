...
def fetch_fmp_financials(ticker, _v=3):
    """Fetch up to 10 years annual + 12 quarters from FMP."""
    base = "https://financialmodelingprep.com/api/v3"
    def get(ep):
        try:
            sep = "&" if "?" in ep else "?"
            r = _requests.get(f"{base}/{ep}{sep}apikey={FMP_API_KEY}", timeout=10)
    ...
    quarterly = {
        "revenue":     to_series(income_q,   "revenue"),
        "netIncome":   to_series(income_q,   "netIncome"),
        "eps":         to_series(income_q,   "eps"),
        "fcf":         to_series(cashflow_q, "freeCashFlow"),
        "grossProfit": to_series(income_q,   "grossProfit"),
        "opIncome":    to_series(income_q,   "operatingIncome"),
        "shares":      to_series(income_q,   "weightedAverageShsOut"),
        "capex":       to_series(cashflow_q, "capitalExpenditure"),
        "deferred":    to_series(balance_q,  "deferredRevenue"),
    }
    for d in [annual, quarterly]:
        if d["capex"] is not None and not d["capex"].empty and d["capex"].mean() < 0:
            d["capex"] = d["capex"] * -1
...
if st.button("Generate Deep Analysis"):
    with st.spinner("Fetching data..."):
        try:
            def _fmp(ep):
                sep = "&" if "?" in ep else "?"
                url = f"https://financialmodelingprep.com/api/v3/{ep}{sep}apikey={FMP_API_KEY}"
                r = _requests.get(url, timeout=10)
                try:
                    data = r.json()
                    # FMP returns {"Error Message": "..."} on auth/plan issues
                    if isinstance(data, dict) and ("Error Message" in data or "message" in data):
                        st.error(f"FMP API error for {ep[:40]}: {data.get('Error Message') or data.get('message')}")
                        return {}
                    return data
                except:
                    st.error(f"FMP parse error {r.status_code}: {r.text[:100]}")
                    return {}

            t = ticker_input
            quote_data   = _fmp(f"quote/{t}")
            profile_data = _fmp(f"profile/{t}")
            ratios_data  = _fmp(f"ratios-ttm/{t}")
            metrics_data = _fmp(f"key-metrics-ttm/{t}")
            pt_data      = _fmp(f"price-target-consensus/{t}")
            hist_raw     = _fmp(f"historical-price-full/{t}")
            news_raw     = _fmp(f"stock_news?tickers={t}&limit=15")
            rec_raw      = _fmp(f"analyst-stock-recommendations/{t}?limit=20")

            # Debug: show what FMP returned
            if not quote_data or quote_data == {}:
                st.warning(f"FMP quote returned empty for {t}. quote_data={str(quote_data)[:200]}")
            if not profile_data or profile_data == {}:
                st.warning(f"FMP profile returned empty. profile_data={str(profile_data)[:200]}")
            ...
            st.info(f"Debug — price: {quote.get('price')}, pe: {quote.get('pe')}, company: {profile.get('companyName')}")

            price    = float(quote.get("price") or 0)
            mkt_cap  = float(quote.get("marketCap") or 0)
            last_div = float(profile.get("lastDiv") or 0)
            div_yield = (last_div / price) if price else 0

            # Revenue growth YoY
            inc2 = _fmp(f"income-statement/{t}?limit=2")
            rev_growth, earn_growth = None, None
            if isinstance(inc2, list) and len(inc2) >= 2:
                r0,r1 = inc2[0].get("revenue",0), inc2[1].get("revenue",0)
                e0,e1 = inc2[0].get("netIncome",0), inc2[1].get("netIncome",0)
                if r1: rev_growth   = (r0-r1)/abs(r1)
                if e1: earn_growth  = (e0-e1)/abs(e1)

            info = {
                "currentPrice":             price,
                "marketCap":                mkt_cap,
                "trailingPE":               quote.get("pe"),
                "forwardPE":                ratios.get("priceEarningsRatioTTM") or quote.get("pe"),
                "trailingEps":              quote.get("eps"),
                "forwardEps":               quote.get("eps"),
                "targetMeanPrice":          pt.get("targetConsensus") or pt.get("targetMean"),
                "fiftyTwoWeekHigh":         quote.get("yearHigh"),
                "fiftyTwoWeekLow":          quote.get("yearLow"),
                "volume":                   quote.get("volume"),
                "averageVolume":            quote.get("avgVolume"),
                "beta":                     profile.get("beta"),
                "shortName":                profile.get("companyName") or t,
                "longName":                 profile.get("companyName") or t,
            ...
            if hist_list:
                ...
                hist_df = hist_df.rename(columns={"close":"Close","open":"Open","high":"High","low":"Low","volume":"Volume"})
                hist_df = hist_df[[c for c in ["Close","Open","High","Low","Volume"] if c in hist_df.columns]]
            else:
            ...
            for n in (news_raw if isinstance(news_raw, list) else []):
                news_list.append({"title": n.get("title",""), "publisher": n.get("site",""),
                                  "link": n.get("url",""), "providerPublishTime": n.get("publishedDate","")})

            # Recommendations
            recs = pd.DataFrame(rec_raw) if isinstance(rec_raw, list) and rec_raw else None

            # FMP 10-year KPI data
            fmp_a, fmp_q = fetch_fmp_financials(t)
            ...
            st.session_state["ticker_loaded"]    = t
...
<script>
  ...
  function switchRange(range) {{
    ...
    with kpi_tab1:
        c1, c2 = st.columns(2)
        with c1:
            _d = fmp_annual.get('revenue')
            if _d is not None:
                ch, bg = bar_chart(_d, "Revenue", "#2ecc71")
                if ch:
                    st.subheader("Annual Revenue")
                    st.altair_chart(ch, use_container_width=True)
                    render_growth_badges(bg)
        with c2:
            _d = fmp_annual.get('netIncome')
            if _d is not None:
                ch, bg = bar_chart(_d, "Net Income", "#3498db")
                if ch:
        ...
        c3, c4 = st.columns(2)
        with c3:
            _d = fmp_annual.get('eps')
            if _d is not None:
                ch, bg = bar_chart(_d, "EPS ($)", "#9b59b6", format_billions=False)
                if ch:
                    st.subheader("EPS")
                    st.altair_chart(ch, use_container_width=True)
                    render_growth_badges(bg)
        with c4:
            _d = fmp_annual.get('fcf')
            if _d is not None:
                ch, bg = bar_chart(_d, "FCF", "#e67e22")
                if ch:
        ...
        c5, c6 = st.columns(2)
        with c5:
            _d = fmp_annual.get('grossProfit')
            if _d is not None:
                ch, bg = bar_chart(_d, "Gross Profit", "#1abc9c")
                if ch:
                    st.subheader("Gross Profit")
                    st.altair_chart(ch, use_container_width=True)
                    render_growth_badges(bg)
        with c6:
            _d = fmp_annual.get('opIncome')
            if _d is not None:
                ch, bg = bar_chart(_d, "Operating Income", "#e74c3c")
                if ch:
        ...
        c7, c8 = st.columns(2)
        with c7:
            shares_data = fmp_annual.get('shares')
    ...
    with kpi_tab2:
        c1, c2 = st.columns(2)
        with c1:
            _d = fmp_quarterly.get('revenue')
            if _d is not None:
                ch, bg = bar_chart(_d, "Revenue", "#2ecc71", quarterly=True)
                if ch:
                    st.subheader("Quarterly Revenue")
                    st.altair_chart(ch, use_container_width=True)
                    render_growth_badges(bg)
        with c2:
            _d = fmp_quarterly.get('netIncome')
            if _d is not None:
                ch, bg = bar_chart(_d, "Net Income", "#3498db", quarterly=True)
                if ch:
        ...
        c3, c4 = st.columns(2)
        with c3:
            _d = fmp_quarterly.get('eps')
            if _d is not None:
                ch, bg = bar_chart(_d, "EPS ($)", "#9b59b6", format_billions=False, quarterly=True)
                if ch:
                    st.subheader("Quarterly EPS")
                    st.altair_chart(ch, use_container_width=True)
                    render_growth_badges(bg)
        with c4:
            _d = fmp_quarterly.get('fcf')
            if _d is not None:
                ch, bg = bar_chart(_d, "FCF", "#e67e22", quarterly=True)
                if ch:
        ...
        c5, c6 = st.columns(2)
        with c5:
            _d = fmp_quarterly.get('grossProfit')
            if _d is not None:
                ch, bg = bar_chart(_d, "Gross Profit", "#1abc9c", quarterly=True)
                if ch:
                    st.subheader("Quarterly Gross Profit")
                    st.altair_chart(ch, use_container_width=True)
                    render_growth_badges(bg)
        with c6:
            _d = fmp_quarterly.get('opIncome')
            if _d is not None:
                ch, bg = bar_chart(_d, "Operating Income", "#e74c3c", quarterly=True)
                if ch:
        ...
        c7, c8 = st.columns(2)
        with c7:
            q_shares_data = fmp_quarterly.get('shares')
