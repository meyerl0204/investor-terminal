...
if st.button("Generate Deep Analysis"):
    with st.spinner("Fetching data..."):
        try:
            ...
            # ── FULL PRICE HISTORY ─────────────────────────────────────────
            st.header("📉 Full Price History (Since IPO)")
            try:
                hist = stock.history(period="max")
                if not hist.empty:
                    hist = hist.reset_index()
                    hist['Date'] = pd.to_datetime(hist['Date'])
                    price_chart = alt.Chart(hist).mark_line(color="#f39c12", strokeWidth=1.5).encode(
                    ...
                    ipo_year = hist['Date'].dt.year.min()
                    st.caption(f"Data from {ipo_year} to present — {len(hist):,} trading days of history")
            except Exception as e:
